...
async function fetchModels(chatbot: ChatBotEnum, options: Record<string, any> = {}) {
  ...
  try {
    ...
    if ((error as Error).name === 'AbortError') {
      ...
      modelValidation.value.message = (error as Error).message || t('aiConfig.form.validation.failed', {}, true);
    }
  }
  ...
  <div class="form-values">
    ...
    <div class="form-field">
      <LabeledSelect
        ...
        :loading="modelValidation.status === ValidationStatus.VALIDATING"
        :disabled="readOnly || modelValidation.status === ValidationStatus.VALIDATING || (!formData[getModelKey(formData[Settings.ACTIVE_CHATBOT])] && !models[formData[Settings.ACTIVE_CHATBOT] as ChatBotEnum]?.length)"
        @update:value="(val: string) => updateValue(getModelKey(formData[Settings.ACTIVE_CHATBOT]), val)"
      />
      <label
        v-clean-html="t(`aiConfig.form.${ getModelKey(formData[Settings.ACTIVE_CHATBOT]) }.description`, {}, true)"
        class="text-label"
      />
      <Banner
        v-if="modelValidation.status === ValidationStatus.ERROR"
        class="m-0"
        color="error"
      >
