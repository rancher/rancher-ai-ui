...
3. Within that context tag, click the `.vs__deselect` button.
...
**Selectors**:
- `[data-testid="rancher-ai-ui-context-tag-local"] .vs__deselect`
- `.context-reset`
...
2. Remove `local` context tag (click `.vs__deselect`).
3. Click the `.context-trigger` button ("Add context").
4. In the dropdown, click the item containing text `local` (the cluster option).
...
- `.rc-dropdown-item` or `[class*="dropdown-item"]` containing "local"
- `[data-testid="rancher-ai-ui-context-tag-local"]`
...
- **`ContextPo`** (`cypress/e2e/po/context.po.ts`):
  ...
  - `removeTag(value: string)` — clicks `.vs__deselect` within the tag
