...
- Menu contains "Keyboard shortcuts" text
...
- `cy.contains('.v-popper__popper', 'Keyboard shortcuts')` — shortcuts option
...
4. Click the "Keyboard shortcuts" option inside the popper
...
**Selectors**:
- `cy.contains('.v-popper__popper', 'Keyboard shortcuts')` — the menu item
...
- The `[data-testid="rancher-ai-ui-delete-chat-confirm-button"]` button is visible

**Selectors**:
- `[data-testid="rancher-ai-ui-chat-container"]` — keyboard target
- `[data-testid="rancher-ai-ui-delete-chat-confirm-button"]` — confirm button
...
- `cy.wait(500)` may be needed between opening the ⋮ menu and clicking an item if menu animation causes flakiness — but prefer `.should('be.visible')` assertions as implicit waits first.
